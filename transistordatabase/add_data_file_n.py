...
def generate_values(temp,lens,n_value):
...
def generate_data(new_list,list2, channel_values,n_value,identifier):
    ...
    channel_values: dictionary containing channel current and voltage values with corresponding (T_junc,gate_V)
    ...
    for j in new_list:
        for i  in list2:
            for key,value in channel_values.items():
                    #print(key[identifier],i,j)
                    if key[identifier] == i:
                        print(key[identifier],i,j)
                        input_voltages = value[0] 
                        current_values = value[1]
                        #print(i,key[identifier])
                        #print("Here")
                        if identifier == 1:
                            replicated_new_gate_voltage = np.repeat(i, len(input_voltages))
                            replicated_junction_temperature = np.repeat(j, len(input_voltages))
                        else:
                            replicated_junction_temperature = np.repeat(j, len(input_voltages))
                            replicated_new_gate_voltage = np.repeat(i, len(input_voltages))
                        replicated_old_gate_voltage = np.repeat(key[identifier], len(input_voltages))
                        ...
                        #print(predicted_current_values_gb)
...
def getplot_V(channel_loss,V_gate_required):
...
# Calculate the number of intervals and the distance between them
    new_junction_temperatures = generate_values(temp,lens,V_gate_required)
    print(new_junction_temperatures)
    required_data,channel_v_updated = generate_data(new_junction_temperatures,gate_v,channel_values,V_gate_required,1)
    return required_data,channel_v_updated
...
def generate_values_T(lst, num_values,n_value):
   
    
    # Extract the minimum and maximum values from the original list
    min_value = min(lst)
    max_value = max(lst)

    # Calculate the number of values needed for equal spacing
    num_intermediate_values = num_values - 2

    # Calculate the step size for the intermediate values
    step = (max_value - min_value) / (num_intermediate_values + 1)

    # Generate the new list with preserved minimum and maximum values
    result = [round(min_value + i * step) for i in range(num_intermediate_values + 1)]

    # Add the maximum value to the list (rounded to the nearest integer)
    result.append(round(max_value))
    result.append(n_value)
...
def getplot_T(channel_loss,updated_channel, n_value):
    #fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    ...
    for i in (channel_loss):
        ...
        channel_values[(i.t_j,i.v_g)] = i.graph_v_i 
# Calculate the number of intervals and the distance between them
    new_gate_voltages = generate_values_T(gate_v,lens[0],n_value)
    print(new_gate_voltages)
    required_data,channel_v_updated = generate_data(new_gate_voltages,temp,updated_channel,n_value,0)
    return required_data,channel_v_updated
...
def get_interpolated_data(V_gate_required,T_junction_required, transistor):
    ...
    data_V, channel_values_V_gate_updated = getplot_V(channel_loss,V_gate_required)
    data_T, updated_channel_T_junc_updated = getplot_T(channel_loss,channel_values_V_gate_updated,T_junction_required)
    #print(data_T,updated_channel_T_junc_updated)
...
get_interpolated_data(10,35,"CREE_C3M0016120K")
