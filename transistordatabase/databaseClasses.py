...
class Transistor:
    ...
    def get_channel_data(self, gate_voltages=list()):
        ...
        try:
            ...
            plecs_transistor = {
                ...
                'TurnOnLoss': {},
                'TurnOffLoss': {},
                'Comment': ["This datasheet was created by {0} on {1} and was exported using transistordatabase.".format(transistor_dict['author'], transistor_dict['datasheet_date'])
                        ,"Datsheet Link : {0}".format(re.sub(r'&', '&amp;', transistor_dict['datasheet_hyperlink'])), "File generated : {0}".format(datetime.datetime.today()),
                            "File generated by : https://github.com/upb-lea/transistordatabase"]
            }
            for channel_dict in transistor_dict['switch']['channel']:
                if channel_dict['v_g'] == v_g_on:
                    try:
                        if limit_current and limit_current > max(channel_dict['graph_v_i'][1]):
                            limit_current = max(channel_dict['graph_v_i'][1])
                    except NameError:
                        limit_current = max(channel_dict['graph_v_i'][1])
            for channel_dict in transistor_dict['switch']['channel']:
                if channel_dict['v_g'] == v_g_on:
                    interp_current = np.linspace(0, limit_current, 20)
                    channel_data = np.interp(interp_current, channel_dict['graph_v_i'][1], channel_dict['graph_v_i'][0])
                    if 'Channel' not in plecs_transistor['ConductionLoss']:
                        plecs_transistor['ConductionLoss']['CurrentAxis'] = interp_current.tolist()
                        plecs_transistor['ConductionLoss']['Channel'] = list()
                        plecs_transistor['ConductionLoss']['TemperatureAxis'] = list()
                    plecs_transistor['ConductionLoss']['TemperatureAxis'].append(channel_dict['t_j']) #forward characteristics are defined only at one gate voltage and doesot depend on v_supply
                    plecs_transistor['ConductionLoss']['Channel'].append(channel_data.tolist())

            #Check if channel information exists else throw exception and dont export transistor xml data
            if 'Channel' not in plecs_transistor['ConductionLoss']:
                raise MissingDataError(1111)
            elif plecs_transistor['type'].lower() in ['mosfet', 'sic-mosfet']:
                plecs_transistor['ConductionLoss']['Channel'], plecs_transistor['ConductionLoss']['CurrentAxis'] = negate_and_append(plecs_transistor['ConductionLoss']['Channel'],plecs_transistor['ConductionLoss']['CurrentAxis'])

            #Turn on loss information extraction
            if 1104 not in exception_codes['Switch']:
                for e_on_dict in transistor_dict['switch']['e_on']:
                    if e_on_dict['v_g'] == v_g_on and e_on_dict['dataset_type'] == 'graph_i_e' and e_on_dict['graph_i_e'] is not None:
                        try:
                            if limit_current_eon and limit_current_eon > max(e_on_dict['graph_i_e'][0]):
                                limit_current_eon = max(e_on_dict['graph_i_e'][0])
                        except NameError:
                            limit_current_eon = max(e_on_dict['graph_i_e'][0])
                for e_on_dict in transistor_dict['switch']['e_on']:
                    if e_on_dict['v_g'] == v_g_on and e_on_dict['dataset_type'] == 'graph_i_e' and e_on_dict['graph_i_e'] is not None:
                        interp_current = np.linspace(0, limit_current_eon, 20)
                        loss_energy = np.interp(interp_current, e_on_dict['graph_i_e'][0], e_on_dict['graph_i_e'][1])
                        if 'Energy' not in plecs_transistor['TurnOnLoss']:
                            plecs_transistor['TurnOnLoss']['CurrentAxis'] = interp_current.tolist()
                            plecs_transistor['TurnOnLoss']['Energy'] = {}
                            plecs_transistor['TurnOnLoss']['TemperatureAxis'] = list()
                        if e_on_dict['v_supply'] not in plecs_transistor['TurnOnLoss']['Energy']:
                            plecs_transistor['TurnOnLoss']['Energy'][e_on_dict['v_supply']] = []
                        plecs_transistor['TurnOnLoss']['Energy'][e_on_dict['v_supply']].append(loss_energy.tolist())
                        # Loss curves are defined at one v_g in many v_supply voltages, therefore to avoid redundancy in Tj and v_supply appends
                        plecs_transistor['TurnOnLoss']['TemperatureAxis'].append(e_on_dict['t_j']) if e_on_dict['t_j'] not in plecs_transistor['TurnOnLoss']['TemperatureAxis'] else None
                if 'Energy' not in plecs_transistor['TurnOnLoss']:
                    ...
                    for key, value in plecs_transistor['TurnOnLoss']['Energy'].items():
                        if not len(plecs_transistor['TurnOnLoss']['Energy'][key]) == len(plecs_transistor['TurnOnLoss']['TemperatureAxis']):
            ...
            if 1105 not in exception_codes['Switch']:
                for e_off_dict in transistor_dict['switch']['e_off']:
                    if e_off_dict['v_g'] == v_g_off and e_off_dict['dataset_type'] == 'graph_i_e' and e_off_dict['graph_i_e'] is not None:
                        try:
                            if limit_current_eoff and limit_current_eoff > max(e_off_dict['graph_i_e'][0]):
                                limit_current_eoff = max(e_off_dict['graph_i_e'][0])
                        except NameError:
                            limit_current_eoff = max(e_off_dict['graph_i_e'][0])
                for e_off_dict in transistor_dict['switch']['e_off']:
                    if e_off_dict['v_g'] == v_g_off and e_off_dict['dataset_type'] == 'graph_i_e' and e_off_dict['graph_i_e'] is not None:
                        interp_current = np.linspace(0, limit_current_eoff, 20)
                        loss_energy = np.interp(interp_current, e_off_dict['graph_i_e'][0], e_off_dict['graph_i_e'][1])
                        if 'Energy' not in plecs_transistor['TurnOffLoss']:
                            plecs_transistor['TurnOffLoss']['CurrentAxis'] = interp_current.tolist()
                            plecs_transistor['TurnOffLoss']['Energy'] = {}
                            plecs_transistor['TurnOffLoss']['TemperatureAxis'] = list()
                        if e_off_dict['v_supply'] not in plecs_transistor['TurnOffLoss']['Energy']:
                            plecs_transistor['TurnOffLoss']['Energy'][e_off_dict['v_supply']] = []
                        plecs_transistor['TurnOffLoss']['Energy'][e_off_dict['v_supply']].append(loss_energy.tolist())
                        # Loss curves are defined at one v_g in many v_supply voltages, therefore to avoid redundancy in Tj and v_supply appends1
                        plecs_transistor['TurnOffLoss']['TemperatureAxis'].append(e_off_dict['t_j']) if e_off_dict['t_j'] not in plecs_transistor['TurnOffLoss']['TemperatureAxis'] else None
                if 'Energy' not in plecs_transistor['TurnOffLoss']:
                    ...
                    for key, value in plecs_transistor['TurnOffLoss']['Energy'].items():
                        if not len(plecs_transistor['TurnOffLoss']['Energy'][key]) == len(plecs_transistor['TurnOffLoss']['TemperatureAxis']):
            ...
            if transistor_dict['switch']['thermal_foster']['r_th_vector'] is not None:
                ...
                plecs_transistor['RElement'] = transistor_dict['switch']['thermal_foster']['r_th_total'] if transistor_dict['switch']['thermal_foster']['r_th_total'] else 1e-6,
                plecs_transistor['TauElement'] = transistor_dict['switch']['thermal_foster']['tau_total'] if transistor_dict['switch']['thermal_foster']['tau_total'] else plecs_transistor['RElement']
        ...
        try:
            ...
            for channel_dict in transistor_dict['diode']['channel']:
                if channel_dict['v_g'] == v_d_on or not is_body_diode :
                    try:
                        if diode_limit_current and diode_limit_current > max(np.abs(channel_dict['graph_v_i'][1])):
                            diode_limit_current = max(np.abs(channel_dict['graph_v_i'][1]))
                    except NameError:
                        diode_limit_current = max(np.abs(channel_dict['graph_v_i'][1]))
            for channel_dict in transistor_dict['diode']['channel']:
                if channel_dict['v_g'] == v_d_on or not is_body_diode :
                    interp_current = np.linspace(0, diode_limit_current, 20)
                    channel_data = np.interp(interp_current, np.abs(channel_dict['graph_v_i'][1]), np.abs(channel_dict['graph_v_i'][0]))
                    if 'Channel' not in plecs_diode['ConductionLoss']:
                        plecs_diode['ConductionLoss']['CurrentAxis'] = interp_current.tolist()
                        plecs_diode['ConductionLoss']['Channel'] = list()
                        plecs_diode['ConductionLoss']['TemperatureAxis'] = list()
                    plecs_diode['ConductionLoss']['TemperatureAxis'].append(channel_dict['t_j'])
                    plecs_diode['ConductionLoss']['Channel'].append(channel_data.tolist())
            if 'Channel' not in plecs_diode['ConductionLoss']:
                raise MissingDataError(1211)
            if 1202 not in exception_codes['Diode']:
                if transistor_dict['diode']['e_rr'] is not None:
                    for e_rr_dict in transistor_dict['diode']['e_rr']:
                        if e_rr_dict['v_g'] == v_d_off and e_rr_dict['dataset_type'] == 'graph_i_e' and e_rr_dict['graph_i_e'] is not None:
                            try:
                                if limit_current_err and limit_current_err > max(e_rr_dict['graph_i_e'][0]):
                                    limit_current_err = max(e_rr_dict['graph_i_e'][0])
                            except NameError:
                                limit_current_err = max(e_rr_dict['graph_i_e'][0])
                    for e_rr_dict in transistor_dict['diode']['e_rr']:
                        if e_rr_dict['v_g'] == v_d_off and e_rr_dict['dataset_type'] == 'graph_i_e' and e_rr_dict['graph_i_e'] is not None:
                            interp_current = np.linspace(0, limit_current_err, 20)
                            loss_energy = np.interp(interp_current, e_rr_dict['graph_i_e'][0], e_rr_dict['graph_i_e'][1])
                            if 'Energy' not in plecs_diode['TurnOffLoss']:
                                plecs_diode['TurnOffLoss']['CurrentAxis'] = interp_current.tolist()
                                plecs_diode['TurnOffLoss']['Energy'] = {}
                                plecs_diode['TurnOffLoss']['TemperatureAxis'] = list()
                            if -e_rr_dict['v_supply'] not in plecs_diode['TurnOffLoss']['Energy']:
                                plecs_diode['TurnOffLoss']['Energy'][-e_rr_dict['v_supply']] = []
                            plecs_diode['TurnOffLoss']['Energy'][-e_rr_dict['v_supply']].append(loss_energy.tolist())
                            # Loss curves are defined at one v_g in many v_supply voltages, therefore to avoid redundancy in Tj and v_supply appends
                            plecs_diode['TurnOffLoss']['TemperatureAxis'].append(e_rr_dict['t_j']) if \
                            e_rr_dict['t_j'] not in plecs_diode['TurnOffLoss']['TemperatureAxis'] else None
                    if 'Energy' not in plecs_diode['TurnOffLoss']:
                        ...
                        for key, value in plecs_diode['TurnOffLoss']['Energy'].items():
                            if not len(plecs_diode['TurnOffLoss']['Energy'][key]) == len(
                                ...
                                raise MissingDataError(1203) #Instead of throwing exception think of loading zero values as turn off losses can also be neglected
            else:
                plecs_diode['TurnOffLoss']['CurrentAxis'] = [0]
                plecs_diode['TurnOffLoss']['Energy'] = {0: [[0]]}
                plecs_diode['TurnOffLoss']['TemperatureAxis'] = [25]
            ...
            if transistor_dict['diode']['thermal_foster']['r_th_vector'] is not None:
                ...
                plecs_diode['RElement'] = transistor_dict['diode']['thermal_foster']['r_th_total'] if transistor_dict['diode']['thermal_foster']['r_th_total'] else 1e-6,
                plecs_diode['TauElement'] = transistor_dict['diode']['thermal_foster']['tau_total'] if transistor_dict['diode']['thermal_foster']['tau_total'] else plecs_diode['RElement']
        except MissingDataError as e:
            print(e.args[0], e.em[e.args[0]])
